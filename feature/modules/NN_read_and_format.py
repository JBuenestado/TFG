...
def feature_file_to_numpyarray(filepath, n_features, label, rows_to_skip=0):
    '''This function takes:
    - filepath: (string) Path to a data file with one row per sample. Each row contains n_features
    real numbers separated by any whitespace (spaces or tabs).
    - n_features: (integer) Number of feature columns per row (e.g. 9 for LU-invariant features).
    - label: (float) Appended as the last element of each row of the output array.
    - rows_to_skip: (integer) Number of leading rows to skip (e.g. headers).

    Unlike file_to_numpyarray(), this function does not assume the file stores flattened density
    matrices. It reads precomputed features (e.g. LU-invariants) directly as columns.

    The function returns a 2D numpy array of shape (n_samples, n_features+1) where the last column
    is the label.'''

    if type(filepath)!=type('') or type(n_features)!=type(1) or type(label)!=type(1.0):
        print('feature_file_to_numpyarray(), Err1')
        return -1
    if n_features<1:
        print('feature_file_to_numpyarray(), Err2')
        return -1

    #Use sep='\s+' to be flexible: works for both space-separated and tab-separated files.
    output_array = pd.read_csv(filepath, sep=r'\s+', header=None, index_col=None,
                                usecols=range(n_features), skiprows=rows_to_skip)
    output_array = np.array(output_array, dtype=float)
    number_of_samples = np.shape(output_array)[0]
    label_column = label*np.ones((number_of_samples,1), dtype=float)
    output_array = np.concatenate((output_array, label_column), axis=1)
    return output_array
