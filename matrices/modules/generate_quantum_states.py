...
def random_int(min, max, rng): #Numero random en un intervalo
    '''This function takes:
...
#def generate_M_random_DMs_file(N, M, filepath, rng, pure=True):
    '''This function takes:
...
def generate_random_DMs_files_negativitywise(neg_resolution, M, N, filepath_root, rng, max_neg = 0.5, transpose_first_subsystem=True, pure=True, L_min=2, L_max=10, tolerance=1e-10):
    '''This function takes:
    ...
    - tolerance: (real scalar) A generated state is taken to be separable (and therefore discarded) whenever its
    negativity is <= tolerance. This prevents separable states (negativity 0) from leaking into the first
    negativity subinterval (0.0, neg_resolution]. A small positive value is needed (instead of testing ==0) because
    a separable 2-qubit state is PPT, so its partial transpose eigenvalues are 0 up to floating point noise.
    ...
    if pure==True:
        while np.prod(files_are_full)!=1:
            ...
            #Discard separable states (negativity ~ 0) so they do not leak into the first subinterval (0.0, neg_resolution]
            if negativity_holder <= tolerance:
                continue
